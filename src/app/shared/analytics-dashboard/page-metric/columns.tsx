...
import { HeaderCell, Badge, Title, Text } from 'shafa-bo';
  ...
  if (trafficShare > 70) {
    return (
      <div className='flex items-center'>
        ...
        <Text className='ms-2 font-medium text-green-dark'>{trafficShare}%</Text>
      </div>
    );
  }
  if (trafficShare > 40) {
    return (
      <div className='flex items-center'>
        ...
        <Text className='ms-2 font-medium text-orange-dark'>{trafficShare}%</Text>
      </div>
    );
  }
  return (
...
export const getColumns = ({ sortConfig, onHeaderCellClick }: Columns) => [
  {
    title: (
      <HeaderCell
        title='Pages'
        sortable
        ascending={sortConfig?.direction === 'asc' && sortConfig?.key === 'pages'}
  ...
  },
  {
    title: (
      <HeaderCell
        title='Traffic Share'
        sortable
        ascending={sortConfig?.direction === 'asc' && sortConfig?.key === 'trafficShare'}
      />
    ),
  ...
  },
  {
    title: (
      <HeaderCell
        ...
        sortable
        ascending={sortConfig?.direction === 'asc' && sortConfig?.key === 'uniquePreviews'}
      />
    ),
    ...
    render: (value: number) => <div className='font-medium'>{formatter.format(value)}</div>,
  },
  {
    ...
    render: (value: Date) => (
      <div className=''>
        ...
        <Text className='text-[13px] text-gray-500'>{dayjs(value).format('DD MMM YYYY')}</Text>
      </div>
    ),
  },
  {
    title: <HeaderCell title='Activity' align='right' className='pe-2 lg:pe-4' />,
    dataIndex: 'chart',
    key: 'chart',
    width: 200,
