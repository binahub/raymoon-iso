...
import { cn, ActionIcon, Button, Popover, Title, Text } from 'shafa-bo';
import Favorite from '@/app/shared/file/manager/favorite';

export default function Grid({ data, onDeleteItem }: { data: object[]; onDeleteItem: (id: string) => void }) {
  return (
...
export function CardItem({ item, onDeleteItem, className }: { item: any; onDeleteItem: (id: string) => void; className?: string }) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <Title as='h4' className='mb-1 truncate text-sm font-medium text-gray-800'>
        {item?.file?.name}
      </Title>
