...
import { Button, Spinner } from 'shafa-bo';
...
export default function FileGrid() {
  const { isLoading, paginatedData, remainingItems, searchTerm, handleDelete, handleSearch, handleLoadMore } = useGrid(
    fileGridData,
    tableDataPerPage
  );
    ...
    <>
      ...
      {remainingItems > 0 && (
        <div className='mt-5 flex flex-col items-center xs:mt-6 sm:mt-8'>
          <Button isLoading={isLoading} onClick={() => handleLoadMore()} className='dark:bg-gray-100 dark:text-white'>
            Load More
          </Button>
        </div>
      )}
