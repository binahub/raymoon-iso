...
import { HeaderCell, Title, Text, Checkbox, ActionIcon, Button, Popover } from 'shafa-bo';
...
export const getColumns = ({ data, sortConfig, checkedItems, onDeleteItem, onHeaderCellClick, handleSelectAll, onChecked }: Columns) => [
  {
    title: (
      <div className='ps-2'>
        <Checkbox className='cursor-pointer' checked={checkedItems.length === data.length} onChange={handleSelectAll} />
      </div>
    ),
    dataIndex: 'checked',
    key: 'checked',
    ...
    render: (_: any, row: any) => (
      <div className='inline-flex ps-2'>
        <Checkbox className='cursor-pointer' checked={checkedItems.includes(row.id)} {...(onChecked && { onChange: () => onChecked(row.id) })} />
      </div>
    ),
  },
  {
    ...
    render: (file: any, row: any) => (
      <div className='flex items-center'>
        <div className='flex h-12 w-12 items-center justify-center rounded-xl bg-gray-100'>
          <Image src={file.avatar} className='aspect-square' width={26} height={26} alt='' />
        </div>
  ...
  },
  {
    ...
    render: (value: Date) => (
      <>
        <Text className='mb-1 text-gray-500'>{dayjs(value).format('DD MMM YYYY')}</Text>
      </>
    ),
  },
  {
