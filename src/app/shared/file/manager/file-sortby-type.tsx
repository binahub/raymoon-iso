...
import { cn, Button, Popover, DocIcon, FolderIcon, ImageIcon, PDFIcon, XMLIcon } from 'shafa-bo';
...
export default function FileSortbyType({
  ...
  return (
    <Popover
      ...
      className={cn('z-50 px-0 dark:bg-gray-100 [&>svg]:dark:fill-gray-100', className)}
      content={({ setOpen }: any) => (
        <div className='w-full max-w-[460px] px-3 pb-2 pt-1 text-left rtl:text-right'>
          <ul className='grid w-full grid-cols-2 gap-x-3'>
            {fileTypeOptions.map((item: { name: string; value: string; id: number; icon?: React.ReactNode }) => (
              <li key={item.id}>
                <Button
                  type='button'
                  variant='text'
    ...
    >
      <Button variant='outline' size='sm' className='text-sm font-normal text-gray-600'>
