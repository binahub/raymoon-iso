...
import { ActionIcon } from 'shafa-bo';
...
export default function ViewSwitcher() {
  ...
  return (
    <div className='flex items-center gap-2 rounded-lg border border-gray-200 p-1.5 px-1.5'>
      ...
      >
        <PiListBullets className={cn('h-5 w-5 transition-colors group-hover:text-gray-900', !isGridLayout && 'text-white')} />
      </ActionIcon>
      <ActionIcon
        size='sm'
        variant='flat'
      ...
      >
        <PiGridFour className={cn('h-5 w-5 transition-colors group-hover:text-gray-900', isGridLayout && 'text-white')} />
      </ActionIcon>
    </div>
  );
}
