...
export function RatingsCount({ rating, totalRatings }: { rating: number; totalRatings?: number }) {
  return (
    <Text as='span' className='inline-flex w-[100px] flex-shrink-0 items-center justify-end gap-1 text-sm text-gray-900'>
...
export default function ListingCard({ product, className, title = `${product.city}, ${product.country}` }: ListingCardProps) {
  const { thumbnail, tag, rating, ratingCount, hostname, features, price, base64 } = product;
  ...
  return (
    <div className={cn(className)}>
      <div className='relative'>
        ...
        {tag && (
          <Text as='span' className='absolute start-5 top-5 rounded-lg bg-white px-2.5 py-1.5 text-xs font-semibold dark:bg-gray-200'>
            {tag}
          </Text>
        )}
      ...
      <div className='pt-3'>
        <div className='mb-1 flex items-center justify-between'>
          <Link href='' className='max-w-[calc(100%-120px)] flex-grow'>
            <Title as='h6' className='truncate font-semibold transition-colors hover:text-primary'>
              {title}
            </Title>
          </Link>
        ...
        <div className='mt-2 flex items-center font-semibold text-gray-900'>
          <del className='pe-1.5 text-[13px] font-normal text-gray-500'>{price.original}</del>
          {price.sale} night
        </div>
      </div>
    </div>
