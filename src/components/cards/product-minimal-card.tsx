...
export default function ProductMinimalCard({ product, className }: ProductProps) {
  const { slug, title, thumbnail, description, price, sale_price, colors = [] } = product;
  ...
  return (
    <div className={className}>
      ...
      <div className='pt-3.5'>
        ...
        <Link href={routes.eCommerce.productDetails(String(slug ?? generateSlug(title)))}>
          <Title as='h6' className='mb-0.5 truncate font-semibold transition-colors hover:text-primary'>
            {title}
          </Title>
        </Link>
        ...
        {/* <div className='mt-3.5 flex items-center font-semibold text-gray-900'>
          ...
          {sale_price && <del className='ps-1.5 text-[13px] font-normal text-gray-500'>{toCurrency(Number(sale_price))}</del>}
        </div> */}
      </div>
    </div>
  );
