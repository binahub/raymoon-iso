...
export default function ReviewCard({ customer, date, message, attachment }: ReviewCardProps) {
  return (
    <div className='border-border-200 border-t py-6 @md:flex @md:items-start'>
      <div className='shrink-0 @md:w-40 @md:pe-4'>
        ...
        <Text className='text-xs text-gray-500 @md:text-sm'>{dayjs(date).format('DD MMM, YYYY')}</Text>
      </div>
        ...
        {attachment && attachment.length > 0 && (
          <div className='grid grid-cols-[repeat(auto-fill,minmax(50px,1fr))] gap-3 pt-2.5'>
            {attachment?.map((item: string, index: number) => (
              <div key={`review-key-${index}`} className='relative mx-auto aspect-[4/5] w-full overflow-hidden rounded bg-gray-100'>
                <Image fill priority src={item} alt='Review attachment' sizes='(max-width: 768px) 100vw' className='h-full w-full object-cover' />
              </div>
            ))}
          </div>
        )}
