...
const tickStyles = {
  ...
  three: 'rtl:[&_.recharts-polar-angle-axis-tick:nth-child(3)]:translate-x-7 rtl:[&_.recharts-polar-angle-axis-tick:nth-child(3)]:translate-y-2',
...
export default function SimpleRadarChart({ data, dataKey, radarKey, fill = '#818cf8', stroke = '#818cf8', className }: SimpleRadarChartTypes) {
  return (
    <ResponsiveContainer width='100%' height='100%'>
      <RadarChart
        className={cn(tickStyles.base, tickStyles.two, tickStyles.three, tickStyles.four, tickStyles.five, tickStyles.six, className)}
        cx='50%'
        cy='50%'
        outerRadius='80%'
        data={data}
      >
        ...
        <PolarRadiusAxis />
        <Radar name='Mike' dataKey={radarKey} stroke={stroke} fill={fill} strokeWidth={1.5} className='fill-opacity-60 dark:fill-opacity-20' />
      </RadarChart>
    </ResponsiveContainer>
  );
}
