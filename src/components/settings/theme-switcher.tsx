...
import { RadioGroup, LightMode, DarkMode } from 'shafa-bo';
...
export default function ThemeSwitcher({}: any) {
  ...
  useEffect(() => {
    if (theme === 'light' && colorPresetName === 'black') {
      updateThemeColor(presetLight.lighter, presetLight.light, presetLight.default, presetLight.dark);
    }
    ...
    <DrawerBlock title='Appearance'>
      ...
      >
        {themeOptions.map((item) => (
          <RadioBox key={item} value={item} className='flex h-auto flex-col justify-center gap-3 rounded-lg p-0 capitalize'>
