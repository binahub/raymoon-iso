...
import { cn, SimpleBar, Badge, Avatar, Title, Text, Popover } from 'shafa-bo';
  ...
  return (
    <div className='w-[320px] text-left rtl:text-right sm:w-[360px] 2xl:w-[420px]'>
      <div className='mb-2 flex items-center justify-between ps-6'>
        ...
        <Link href={routes.support.inbox} onClick={() => setIsOpen(false)} className='hover:underline'>
          View all
        </Link>
      </div>
          ...
          {data.map((item) => (
            ...
            >
              <div className={cn('relative', item.avatar.length > 1 && 'me-1')}>
                <Avatar src={item.avatar[0]} name={item.name} className={cn(item.avatar.length > 1 && 'relative -end-1 -top-0.5 !h-9 !w-9')} />
              ...
              <div className='grid grid-cols-[minmax(0,1fr)_auto] items-center'>
                <div className='w-full'>
                  ...
                  <div className='flex'>
                    ...
                    <span className='ms-auto whitespace-nowrap pe-8 text-xs text-gray-500'>{dayjs(item.sendTime).fromNow(true)}</span>
                  </div>
                </div>
